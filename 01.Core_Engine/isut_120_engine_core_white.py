...
[Summary]
  ...
  4. Visualization: Academic paper style rendering (White background).
...
# [Configuration] Academic Plot Style Settings
plt.rcParams.update({
    'font.size': 12,
...
def mode_animate_with_audit(units: Units, gparams: GalaxyParams, iparams: ISUTParams):
    ...
    # Visualization Setup (Academic Style)
    ...
    ax_sim.set_title("ISUT Disk Simulation (Test Particles)")
    
    # Visualization: Black particles for academic print
    stars_plot, = ax_sim.plot([], [], ".", color='black', markersize=3, alpha=0.7)
    stats_text = ax_sim.text(0.02, 0.95, "", transform=ax_sim.transAxes, fontsize=10, family="monospace", va="top", color='black')
    ...
    ax_curve.plot(r_theory, vN, '--', alpha=0.9, label="Newtonian", color='gray')
    ax_curve.plot(r_theory, vI, linewidth=2.0, label=f"ISUT ({iparams.accel_form})", color='blue')
    ax_curve.set_xlabel("Radius r")
    ax_curve.set_ylabel("Velocity v")
    ax_curve.set_title("Theoretical Rotation Curve")
    ax_curve.legend()

    # Particle Velocity Scatter (Real-time check)
    particles_scatter, = ax_curve.plot([], [], "o", color='black', markersize=1.5, alpha=0.3, label="Particles (r,v)")

    # [Panel 3] Energy Stability Monitor (Bottom)
    ax_energy = plt.subplot(gs[1, :])
    ax_energy.set_xlabel("Time Step"); ax_energy.set_ylabel("Total Energy (Proxy)")
    ax_energy.set_title("Energy Conservation Audit (E = T + V)")
    energy_line, = ax_energy.plot([], [], label="Total Energy", color='crimson', linewidth=1.5)
    ax_energy.legend()
...
def mode_rotation_curve_export(units: Units, gparams: GalaxyParams, iparams: ISUTParams):
    ...
    plt.plot(r, vI, '-', label=f"ISUT ({iparams.accel_form})", color='black', linewidth=2.5)
...
def mode_btfr_check(units: Units, gparams: GalaxyParams, iparams: ISUTParams):
    ...
    plt.scatter(V, M, color='black', s=50, label='ISUT Simulated Galaxies')
    
    v_guide = np.linspace(min(V), max(V), 100)
    m_guide = 10**intercept * v_guide**4 
    plt.plot(v_guide, m_guide, 'r--', alpha=0.8, label="Slope 4 Ref")
